...
function getNextFriday(date) {
  let diffDays = 5 - date.getUTCDay();

  if (diffDays <= 0) {
    diffDays += 7;
  }
  const nextFriday = date.valueOf() + diffDays * 24 * 3600 * 1000;
...
function getCountDaysInMonth(month, year) {
  ...
  const diff = (dateEnd.valueOf() - dateStart.valueOf()) / 24 / 3600 / 1000;
  return diff;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (timestampEnd - timestampStart) / 24 / 3600 / 1000 + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const daysCount = (date.valueOf() - startDate.valueOf()) / 24 / 3600 / 1000;
  const weekCount =
    Math.ceil((daysCount + startDate.getDay()) / 7) +
    (startDate.getDay() ? 0 : 1);
  return weekCount;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const dates = [];
  const startArray = period.start.split('-');
  [startArray[0], startArray[1]] = [startArray[1], startArray[0]];
  const startDate = new Date(startArray.join('-'));

  const endArray = period.end.split('-');
  [endArray[0], endArray[1]] = [endArray[1], endArray[0]];
  const endDate = new Date(endArray.join('-'));

  let workingDayCount = 0;
  while (startDate.valueOf() <= endDate.valueOf()) {
    if (workingDayCount < countWorkDays) {
      dates.push(
